...
func TestSmallBloomFilter(t *testing.T) {
	...
	for _, word := range [][]byte{
		...
		hash = append(hash, word)
	}

	f := NewBloomFilterPoliy(10)

	var buf []byte
	buf = f.AppendFilter(hash, buf)
	got := (_xx)(buf).String()
...
loop:
	for length := 1; length <= 10000; length = nextLength(length) {
		...
		var buf []byte
		f := NewBloomFilterPoliy(10)
		buf = f.AppendFilter(hashes, buf)

		if len(buf) > (length*10/8)+40 {
			t.Errorf("length=%d: len(f)=%d is too large", length, len(buf))
			continue
		}

		// All added keys must match.
		for _, key := range keys {
			if !f.KeyMayMatch(key, buf) {
		...
		nFalsePositive := 0
		for i := 0; i < 10000; i++ {
			if f.KeyMayMatch(le32(1e9+i), buf) {
				nFalsePositive++
			}
		}
